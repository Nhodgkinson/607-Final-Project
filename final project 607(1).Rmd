...
increase_hw_df<-hos_unicef_df %>%
  subset(Year > 1999 & Year < 2006)%>%
  filter(`COUNTRY, AREA OR TERRITORY` == "Namibia"|`COUNTRY, AREA OR TERRITORY` == "Ghana"|`COUNTRY, AREA OR TERRITORY` == "Rwanda"|`COUNTRY, AREA OR TERRITORY` == "Saint Kitts and Nevis"|`COUNTRY, AREA OR TERRITORY` == "Trinidad and Tobago")

increase_hw_df<-increase_hw_df[,c(1:3,8)]

```

```{r}
#pick out 5 countries that had an increase in hand washing over 5 years and 5 countries that saw no change and all countries that saw a decrease

decrease_hw_df<-hos_unicef_df %>%
  subset(Year > 1999 & Year < 2006)%>%
  filter(`COUNTRY, AREA OR TERRITORY` == "Zambia"|`COUNTRY, AREA OR TERRITORY` == "Kenya")

decrease_hw_df<-decrease_hw_df[,c(1:3,8)]
...
same_hw_df<-hos_unicef_df %>%
  subset(Year > 1999 & Year < 2006)%>%
  filter(`COUNTRY, AREA OR TERRITORY` == "Kiribati"|`COUNTRY, AREA OR TERRITORY` == "Lithuania"|`COUNTRY, AREA OR TERRITORY` == "Myanmar"|`COUNTRY, AREA OR TERRITORY` == "Mauritania"|`COUNTRY, AREA OR TERRITORY` == "Egypt")

same_hw_df<-same_hw_df[,c(1:3,8)]
